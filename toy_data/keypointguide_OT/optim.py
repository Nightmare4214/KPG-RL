...
def cg(a, b, C1, C2, constC, f, df,  G0=None, lam=0.1, Mask=None, OT_algorithm="sinkhorn", max_iter_sinkhorn=1000, numItermax=5, numItermaxEmd=100,
    ...
    while loop:
        ...
        if OT_algorithm == "sinkhorn":
            if Mask is None:
                Gc = sinkhorn_log_domain(
                    a, b, Mi, reg=lam, niter=max_iter_sinkhorn)
            else:
                Gc = sinkhorn_log_domain(
                    a, b, Mi, reg=lam, Mask=Mask, niter=max_iter_sinkhorn)
        if OT_algorithm == "lp":
            if Mask is None:
                Gc = lp(a.numpy(), b.numpy(), Mi.numpy())
            else:
                Gc = torch.Tensor(
                    lp(a.numpy(), b.numpy(), Mi.numpy(), Mask=Mask.numpy()))

        deltaG = Gc - G

        # line search
        if Mask is None:
            alpha1, fc, f_val = solve_linesearch(
                cost, G, deltaG, C1, C2, constC)
        else:
            alpha1, fc, f_val = solve_linesearch(
                cost, Mask*G, Mask*deltaG, C1, C2, constC)
