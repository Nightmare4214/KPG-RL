...
def calc_coeff(iter_num, high=1.0, low=0.0, alpha=10.0, max_iter=10000.0):
    ...
    return 2.0 * (high - low) / (1.0 + np.exp(-alpha*iter_num / max_iter)) - (high - low) + low
...
class SLR_layer(nn.Module):
    ...
    def forward(self, input):
        r = 0*input.norm(dim=1).detach()[0]
        cosine = F.linear(input, F.normalize(
            self.weight), r*torch.tanh(self.bias))
        output = cosine
        return output
...
class SP_layer(nn.Module):
    ...
    def exp(self, v, o=None, r=1.0):
        ...
        if o is None:
            o = torch.cat([torch.zeros(1, v.size(1)), r *
                          torch.Tensor([[1]])], dim=1).to(device)
    ...
    def log(self, x, o=None, r=1.0):
        if x.is_cuda == True:
            ...
            device = torch.device('cpu')
        if o is None:
            o = torch.cat([torch.zeros(1, x.size(1) - 1), r *
                          torch.Tensor([[1]])], dim=1).to(device)
    ...
    def shrink(self, x, epsilon=1e-4):
        x[torch.abs(x) > (1 - epsilon)] = x[torch.abs(x)
                                            > (1 - epsilon)] * (1 - epsilon)
        return x
