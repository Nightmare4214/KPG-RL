...
class ImageList(Dataset):
    def __init__(self, image_list, labels=None, transform=None, target_transform=None, mode='RGB', second=False, root=None):
...
class ClassAwareSamplingSTDataset(Dataset):
    def __init__(self, source_list, target_list, transform=None, num_class=31, num_sample_per_class=10, dataset_length=None, root=None):
        ...
        self.source_classaware_list = self.get_classwise_list(
            self.source_path_list, self.source_labels)
        self.target_path_list, self.target_labels = self.get_list(target_list)
        self.target_classaware_list = self.get_classwise_list(
            self.target_path_list, self.target_labels)
        ...
        if self.dataset_length is not None:
            self.selected_classes = np.random.choice(
                self.selected_classes, self.dataset_length)
    ...
    def get_classwise_list(self, image_path_list, labels):
        ...
        for k in range(self.num_class):
            dic[k] = [(image_path_list[i], labels[i])
                      for i in range(len(labels)) if labels[i] == k]
        return dic
    ...
    def __getitem__(self, index):
        ...
        source_idxs = np.random.choice(np.arange(
            len(self.source_classaware_list[c])), self.num_sample_per_class, replace=True)
        target_idxs = np.random.choice(np.arange(
            len(self.target_classaware_list[c])), self.num_sample_per_class, replace=True)
...
def make_dataset(image_list, labels):
    if labels:
        ...
        if len(image_list[0].split()) > 2:
            images = [(val.split()[0], int(val.split()[1]), float(
                val.split()[2]), float(val.split()[3])) for val in image_list]
        else:
