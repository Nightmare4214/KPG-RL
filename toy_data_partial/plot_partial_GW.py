...
import os
import torch
from keypointguide_POT import partial_GW
...
J = [3, num+5]
s = 2/3
_, pi_ = partial_GW.partial_GW(torch.Tensor(C1), torch.Tensor(
    C2), torch.Tensor(p), s*torch.Tensor(q), I, J, s=2/3)
pi_ = pi_.numpy()
pi = pi_[:, :-1]
